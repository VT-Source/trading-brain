...
# ============================================================
# LOGIQUE ANALYSE MASSIVE v5.2.0
# Nouveautés v5.2.0 :
#   - regime_marche : BULL / BEAR / NEUTRE (feature contextuelle macro)
#   - Basé sur la position du prix vs SMA50 du ticker
# ============================================================
 
def run_analysis_logic():
    if engine is None: return
    print("🚀 Démarrage de l'analyse v5.2.0 (AT → ML régime → Log)...")
    ...
    try:
        ...
        for i in range(0, len(all_tickers), chunk_size):
            ...
            def compute_group(group):
                ...
                group['bb_position'] = (price - group['bb_lower']) / (bb_range + 1e-9)
 
                # --------------------------------------------------------
                # APPROCHE 2 — Feature régime de marché v5.2.0
                # --------------------------------------------------------
                # SMA50 : tendance intermédiaire du ticker
                sma_50 = price.rolling(50, min_periods=1).mean()
 
                # Régime basé sur la position du prix vs SMA50
                # BULL   : prix > SMA50 +2% → tendance haussière confirmée
                # BEAR   : prix < SMA50 -2% → tendance baissière confirmée
                # NEUTRE : entre les deux   → marché sans direction claire
                group['regime_marche'] = np.where(
                    price > sma_50 * 1.02, 'BULL',
                    np.where(
                        price < sma_50 * 0.98, 'BEAR',
                        'NEUTRE'
                    )
                )
                ...
                # Target ML
                group['prix_futur'] = price.shift(-20)  # J+20
                group['target_ml']  = (
                    (group['signal_achat']) &
