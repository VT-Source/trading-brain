# ============================================================
# backtest.py — Backtest Signal v3.5b | Trading Brain
# VT-Source | Phase 1 Étape 2
# ============================================================
# Appel : GET /run-backtest?tickers=NVDA,AAPL,MSFT,AMZN,ASML
#
# Ce que ce module teste :
#   1. Signal v3.5b (momentum R² + breakout 50j + RVOL>1.5 + OBV)
#   2. Sortie : Trailing Stop ATR (Chandelier Exit) — k = 2.0/2.5/3.0/3.5/4.0
#   3. Comparaison : signaux en secteur fort vs hors secteur fort
#   4. Critère de validation : Sharpe Ratio > 1.0 sur 3+ tickers
#
# Changements v3.5 → v3.5b :
#   - RVOL seuil : 2.0 → 1.5 (plus de signaux, filtre moins strict)
#   - Breakout : 20j → 50j (capture les vrais breakouts de range)
#   - Cooldown : 5 jours entre signaux (élimine doublons consécutifs)
#   - K_VALUES : ajout de 4.0 (test trailing stop plus large)
#
...
K_VALUES           = [2.0, 2.5, 3.0, 3.5, 4.0]   # multiplicateurs ATR à tester
MOMENTUM_WINDOW    = 252                      # 12 mois de trading
ROC_WINDOW         = 252                      # rendement annuel
BREAKOUT_WINDOW    = 50                       # plus haut 50j (était 20j en v3.5)
RVOL_THRESHOLD     = 1.5                      # conviction volume (était 2.0 en v3.5)
...
COOLDOWN_DAYS      = 5                        # jours minimum entre deux signaux
...
def load_secteur_force(ticker: str) -> pd.DataFrame:
    ...
    Stratégie de jointure (fix 2026-03) :
        1. Récupère secteur + pays depuis tickers_info
        2. Déduit la zone depuis le pays :
           Belgium → 'BE' (utilise ETF EU du même secteur)
           Pays européens connus → 'EU'
           Reste (dont US) → 'US'
        3. Cherche l'ETF sectoriel correspondant (zone déduite, puis US en fallback)
    """
    if engine is None:
        return pd.DataFrame()

    # Liste des pays européens couverts par les ETF EU (STOXX 600)
    PAYS_EU = {
    ...
    try:
        # 1. Récupérer secteur et pays du ticker
        with engine.connect() as conn:
        ...
        if pays == 'Belgium':
            zone_priority = ['EU', 'US']
        elif pays in PAYS_EU:
            zone_priority = ['EU', 'US']
        else:
            zone_priority = ['US', 'EU']

        # 3. Chercher l'ETF sectoriel dans l'ordre de priorité
        for zone in zone_priority:
            with engine.connect() as conn:
                df = pd.read_sql(text("""
                    ...
                    WHERE se.secteur_yahoo = :secteur
                      AND se.zone = :zone
                      AND se.actif = TRUE
                    ORDER BY sep.date ASC
                """), conn, params={"secteur": secteur_yahoo, "zone": zone})
        ...
        # Aucune donnée trouvée dans aucune zone
        return pd.DataFrame()
...
def compute_atr(df: pd.DataFrame, window: int = ATR_WINDOW) -> pd.Series:
    ...
    if has_hl:
        ...
        tr = close.diff().abs()
        print(f"      ATR mode : approché (Close-Close) — lancer /fill-high-low")

    return tr.rolling(window, min_periods=1).mean()


def apply_cooldown(signal: pd.Series, cooldown_days: int = COOLDOWN_DAYS) -> pd.Series:
    """
    Élimine les signaux consécutifs trop proches.
    Après un signal TRUE, les signaux suivants sont forcés à FALSE
    pendant cooldown_days jours.

    Résout le problème des doublons (ex. signaux 25 et 26 mai 2023)
    qui polluent le comptage sans ajouter de valeur.
    """
...
def compute_signals_v35(df: pd.DataFrame, df_secteur: pd.DataFrame) -> pd.DataFrame:
    """
    Calcule tous les indicateurs v3.5b et le signal d'achat composite.

    Niveaux implémentés :
        Niveau 1 : prix > SMA_200
        Niveau 2 : secteur en force relative (si données dispo)
        Niveau 3 : momentum_r2 > 0 + breakout_50j + RVOL > 1.5 + OBV accumulation
        + Cooldown 5 jours entre signaux
    """
    ...
    # Breakout plus haut 50j (était 20j en v3.5)
    breakout = price >= price.rolling(BREAKOUT_WINDOW, min_periods=1).max()

    # RVOL > 1.5 (était 2.0 en v3.5)
    ...
    # --- Cooldown : espace les signaux de 5 jours minimum ---
    signal_achat = apply_cooldown(signal_brut, COOLDOWN_DAYS)
    ...
    result["signal_brut"]     = signal_brut
...
def run_backtest_ticker(ticker: str) -> dict:
    ...
    Teste les 5 valeurs de k ATR et retourne les métriques pour chaque.
    """
    ...
    nb_signaux_brut = int(df_signals["signal_brut"].sum())
    print(f"      Signaux bruts : {nb_signaux_brut} | Après cooldown : {nb_signaux}")

    if nb_signaux == 0:
        return {
            "ticker"         : ticker,
            "nb_signaux"     : 0,
            "nb_signaux_brut": nb_signaux_brut,
            "message"        : "Aucun signal v3.5b sur la période.",
            "resultats_k"    : {}
        }
...
def synthesize_results(all_results: list) -> dict:
    ...
    return {
        ...
        "recommandation" : (
            f"✅ Signal v3.5b VALIDÉ — k={best_k_global} recommandé ({nb_valides} tickers Sharpe>1.0)"
            if signal_valide else
            f"⚠️ Signal v3.5b NON VALIDÉ — seulement {nb_valides}/3 tickers Sharpe>1.0 avec k={best_k_global}"
        )
    }
...
def run_backtest_logic(tickers: list[str] = None, horizon: int = 30) -> dict:
    ...
    print(f"🚀 Backtest v3.5b démarré — {len(tickers)} tickers, horizon={horizon}j")
    print(f"   Tickers : {', '.join(tickers)}")
    print(f"   K ATR testés : {K_VALUES}")
    print(f"   Paramètres v3.5b : RVOL>{RVOL_THRESHOLD} | Breakout {BREAKOUT_WINDOW}j | Cooldown {COOLDOWN_DAYS}j")
    ...
    print(f"\n{'='*60}")
    print(f"📋 SYNTHÈSE BACKTEST v3.5b")
    print(f"{'='*60}")
